module.exports = function ({ api, models, Users, Threads, Currencies }) {
  ...
  return async function ({ event }) {
    ...
    const { allowInbox, PREFIX, ADMINBOT, NDH, DeveloperMode, adminOnly, keyAdminOnly, ndhOnly, ibrieng, DANGKY, spamOnly } = global.config;
  ...
  return api.sendMessage("[⚜️] MIRAI [⚜️] - Chỉ ADMINBOT mới được sử dụng bot trong chat riêng.", threadID, messageID)
    ...
    if (!ADMINBOT.includes(senderID) && adminbot.adminOnly == true) {
     ...
     if (!NDH.includes(senderID) && !ADMINBOT.includes(senderID) && adminbot.ndhOnly == true) { 
          ...
          return api.sendMessage('[⚜️] MIRAI [⚜️] - Chỉ SUPPORTBOT mới có thể sử dụng bot', threadID, messageID)
...
const dataAdbox = require('./../../modules/commands/cache/data.json');
   ...
   if (!command) {
    ...
    if (command.config.hasPermssion > permssion) return api.sendMessage(`[⚜️] Lệnh ${command.config.name} Chỉ Được Dùng Với Những Người Có Quyền Hạn Là ${quyenhan}`, event.threadID, event.messageID);
