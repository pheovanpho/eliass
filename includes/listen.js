...
(async function () {
    ...
    try {
      ...
      for (const dataU of users) {
    	...
    	logger.loader(`BOT NĐH BẮT ĐẦU NHẬN LỆNH`);
    } catch (error) {
  ...
  ╚═╝░░╚═╝╚═╝╚══════╝░╚═════╝░
`, "NĐH");
  logger(`${global.config.ADMINBOT} - JRT`, "ADMINBOT INFO");
  logger(`${api.getCurrentUserID()} - » ${global.config.PREFIX} « • ${(!global.config.BOTNAME) ? "This bot was made by NĐH" : global.config.BOTNAME}`, "BOT INFO");
