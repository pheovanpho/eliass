module.exports.config = {
	name: "bannerfb", // Tên lệnh, được sử dụng trong việc gọi lệnh
...
module.exports.handleReply = async ({ api, event, handleReply }) => {
	...
	switch (handleReply.step) {
		...
		case 6:
			...
			api.unsendMessage(handleReply.messageID);
			let c = content;
			let res = await axios.get(encodeURI(`https://api-rosie.jrt-official.repl.co/fbcover/v1?name=${c.name}&uid=${c.id}&address=${c.address}&email=${c.email}&subname=${c.subname}&sdt=${c.number}&color=${c.color}`), { responseType: "arraybuffer" })
