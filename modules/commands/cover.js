...
module.exports.run = async function({ api, args, event, permssion }) {
  ...
  if(args[0] == "find" || args[0] == "tìm"){
     const lengthchar = (await axios.get('https://api-rosie.jrt-official.repl.co/taoanhdep/data')).data
  ...
  if(!args[0]){
  const abcxyz = ["https://i.imgur.com/ZN2VVYR.png","https://i.imgur.com/7CC8AZo.png","https://imgur.com/bsaJZXb.png","https://imgur.com/2e0NamD.png","https://imgur.com/kPCjQY8.png","https://imgur.com/Ar5DNfT.png"]
  let o = [];
  for(let i = 0; i < 6; i++){
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  if (handleReply.step == 1) {
    ...
    ctx.restore();
    ctx.save();
    Canvas.registerFont(__dirname + `/cover/MTOJamai.ttf`, {
      family: "MTOJamai"
    });
    ctx.fillStyle = "#fff"
  ...
  Canvas.registerFont(__dirname + `/cover/gantellinesignature-bw11b.ttf`, {
    ...
    ctx.fillText(tenchinh, 2100, 870);
    ctx.beginPath();
    const imageBuffer = canvas.toBuffer();
     fs.writeFileSync(pathImg, imageBuffer);
  return api.sendMessage({
