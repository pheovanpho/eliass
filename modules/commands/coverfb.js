	  ...
	  var color = handleReply.color.toUpperCase()
       api.unsendMessage(handleReply.messageID);
            ...
            setTimeout(() => {
                ...
                return request(encodeURI(`https://api-rosie.jrt-official.repl.co/fbcover/v1?name=${name}&uid=${senderID}&address=${address}&email=${email}&subname=${subname}&sdt=${phoneNumber}&color=${color}`)).pipe(fs.createWriteStream(__dirname + '/cache/fbcover.png')).on('close', () => callback());
            }, 1000);
          }, messageID);
        }
    }} 