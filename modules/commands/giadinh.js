...
var one = senderID, two = e, three = r;
    return makeImage({ one, two, three }).then(path => api.sendMessage({ body: `[❤️] Chúc mừng ${nameSender} đã ghép đôi thành công -${coindown}\n[💓] Chúc mừng bạn đã có một gia đình hạnh phúc với ${name1} và có đứa con đầu lòng là ${name2}\n[⚜️] Tỉ lệ hợp đôi của bạn với ${name1} là ${tle}\n[💖] Tỉ lệ hạnh phúc của gia đình bạn là ${tle1}`,mentions: arraytag, attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }
      }