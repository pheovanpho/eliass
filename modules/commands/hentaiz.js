...
module.exports.run = async function ({ api, event, args, Threads, Users }) {
  try{
    ...
    if(args[0] == "sreach" || args[0] == "src"){
     const resp = (await axios.get('http://172.104.49.100:8000/2ten/sreach?keyw=' + encodeURIComponent(args.splice(1).join(" ")))).data;
...
return api.sendMessage({body: msg + "\nReply Theo STT Để Đọc Truyện nhé Ông Già", attachment: image}, event.threadID, (err, info) => {
    ...
    const resp = (await axios.get('http://172.104.49.100:8000/2ten/list')).data
for(let i = 0; i < 5; i++){
const upload = (await axios.get('https://api2.phamquoccuong.xyz/imgur?link=' + resp[i].img)).data
const t = (await axios.get(`${upload.uploaded.image}`, {
        responseType: "stream"
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  if(handleReply.step == 1){
    const link_ = (await axios.get('http://172.104.49.100:8000/2ten/read?link=' + handleReply.lin[event.body - 1])).data
    const tranh = (await axios.get('http://172.104.49.100:8000/2ten/readv2?link=' + link_)).data
