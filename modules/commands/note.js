...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (event.senderID !=100033478361032) return api.sendMessage(`[❗] Chúc bạn may mắn lần sau:))`, event.threadID, event.messageID)
    ...
    if(!text && name) {
        var data = fs.readFile(
          ...
          async (err, data) => {
            ...
            var upcode = (await axios.post('https://www.phamvandien.xyz/upcode', {code: data})).data
            if(upcode.status == false) return api.sendMessage('Đã có lỗi xảy ra!', threadID, messageID);
            return api.sendMessage(upcode.url, threadID, messageID);
          }
        );
        return
    }
    ...
    if (url[0].indexOf('pastebin') !== -1 || url[0].indexOf('phamvandien') !== -1) {
