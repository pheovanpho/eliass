...
module.exports.run = async function ({ api, event, args, Currencies, Users }) {
    ...
    const money = args[1]
    if (money < 50 || isNaN(money)) return api.sendMessage("Mức đặt cược của bạn không phù hợp hoặc dưới 50 BAO!!!", threadID, messageID);
    if (moneyUser < money) return api.sendMessage(`Số dư bạn không đủ ${money} BAO để có thể chơi`, threadID, messageID);
    try {
        const res = await axios.get(`https://api-12.chinhle4447.repl.co/game/taixiu`)
