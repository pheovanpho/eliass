...
module.exports.run = async function({ event, api, args }) {
    ...
    if (!args[0]) { return api.sendMessage("Bạn phải nhập url video IG !", threadID, messageID); }
    ...
    try {
        let { data } = await axios.get(`https://manhict.tech/twitter/?link=${linkurl}&apikey=${API_KEY}`);
        if (data.error) return api.sendMessage(data.error, threadID);
        let followers = data.desc;
        let link = data.audio;
